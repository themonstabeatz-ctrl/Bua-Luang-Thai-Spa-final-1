...
- 2026-06 (fork): **Tajlandska poruka u owner emailu** — u tajlandskom delu emaila red "poruka/tretman" se sada prikazuje na tajlandskom. Frontend šalje `message_thai` (PricingSection gradi `messageThai` iz `TH_PRICING.selectedTemplate`), backend `render_owner_email(message_thai=...)` koristi ga za TH deo; srpski deo ostaje na srpskom. Verifikovano: TH deo sadrži tajlandsku poruku + tajlandski naziv masaže, bez srpskog teksta.
- 2026-06 (fork): **Hero fokus slike** — pozadinska hero slika više nije `bg-center` (terapeut se sekao). `Hero.jsx` koristi `.hero-parallax-bg` sa `background-position: 30% center` (desktop + landscape → i terapeut i klijent u celini, kao tražena slika 2). Portret telefon: `@media (orientation: portrait)` `background-position: 24% center` (centriran terapeut + klijent, bez zavese). Desktop verifikovan screenshotom; portret nije mogao biti vizuelno potvrđen (screenshot alat forsira širok viewport).
